...
export async function crearTurno(req, res, next) {
  ...
  try {
    ...
    const {
      sucursal_id, barbero_id, cliente_id,
      servicio_id, inicio, notas,
    } = req.body;

    // Calcular fin estimado desde duración del servicio
    const [[srv]] = await conn.execute(
      'SELECT duracion_min, precio FROM servicios WHERE id = ?', [servicio_id]
    );
    if (!srv) throw Object.assign(new Error('Servicio no encontrado'), { status: 404 });

    const finEstimado = new Date(new Date(inicio).getTime() + srv.duracion_min * 60_000);
    ...
    const [result] = await conn.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [sucursal_id, barbero_id, cliente_id || null, servicio_id,
        estadoReservadoId, inicio, finEstimado, srv.precio, notas || null]);
...
export async function cambiarEstado(req, res, next) {
  ...
  try {
    ...
    const [[turno]] = await conn.execute(
      'SELECT estado_id FROM turnos WHERE id = ?', [id]
    );
    if (!turno) throw Object.assign(new Error('Turno no encontrado'), { status: 404 });
...
export async function cerrarTurno(req, res, next) {
  ...
  try {
    ...
    const [[turno]] = await conn.execute(
      'SELECT sucursal_id, estado_id FROM turnos WHERE id = ?', [id]
    );
    if (!turno) throw Object.assign(new Error('Turno no encontrado'), { status: 404 });
    if (turno.estado_id === 4) throw Object.assign(new Error('Turno ya completado'), { status: 400 });
    ...
    for (const p of productos) {
      ...
      await conn.execute(
        `INSERT INTO movimientos_stock
         ...
         VALUES (?,?,?,?,?,?)`,
        [turno.sucursal_id, p.producto_id, tipoMov, p.cantidad, id, req.user.id]
      );
    }
